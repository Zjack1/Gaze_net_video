...
camera_matrix_raw = [640., 0., 320.,0., 640., 240.,0., 0., 1.]
camera_matrix = np.array(camera_matrix_raw).reshape(3, 3)

dist_coefficients_raw = [0., 0., 0., 0., 0.]
dist_coefficients = np.array(dist_coefficients_raw).reshape(-1, 1)
...
def estimate_head_pose(landmarks):
        ...
        _, rvec, tvec = cv2.solvePnP(LANDMARKS,
                                     landmarks,
                                     camera_matrix,
                                     dist_coefficients,
                                     rvec,
        ...
        head_pose_rot = rot
        head_position = tvec
        reye_head_pose_rot = rot
        leye_head_pose_rot = rot

        head_pose_result = [head_pose_rot,head_position,reye_head_pose_rot,leye_head_pose_rot]
        return head_pose_result


def compute_3d_pose(head_pose_result) :
        """Compute the transformed model."""
        rot = head_pose_result[0].as_matrix()
        model3d = LANDMARKS @ rot.T + face.head_position

def compute_face_eye_centers(face):
        ...
        face.reye.center = face.model3d[REYE_INDICES].mean(axis=0)
        face.leye.center = face.model3d[LEYE_INDICES].mean(axis=0)
